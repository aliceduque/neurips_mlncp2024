...
def get_train_loader(database, root, device, reduced=False):
    raw_dataset = get_dataset(database, root)
    img = dataset_transformation(database, raw_dataset, device)
    target = raw_dataset.targets.clone().detach().float().to(device)
    raw_tensor = TensorDataset(img,target)


    dataset_size = len(raw_tensor)
    val_size = int(0.2 * dataset_size)
    train_size = dataset_size - val_size
    train_dataset, val_dataset = random_split(raw_tensor, [train_size, val_size])
...
def get_test_loader(database, root, device, reduced=False):
    raw_dataset = get_dataset(database, root, train=False)
    img = dataset_transformation(database, raw_dataset, device)
    target = raw_dataset.targets.clone().detach().float().to(device)
    if reduced:
...
def get_dataset(database, root, train=True, transform=None,
    ...
    if database == 'MNIST':
      return datasets.MNIST(root=path, train=train, transform=transform,
                            download=download)
    elif database == 'FashionMNIST':
        return datasets.FashionMNIST(root=path, train=train, transform=transform, download=download)
...
def dataset_transformation(database, raw_dataset, device):
    if database == 'MNIST':
      img = transforms.Normalize((0.1305,), (0.3081,))((raw_dataset.data/255).clone().detach().float().to(device))
    elif database == 'FashionMNIST':
      img = transforms.Normalize((0.2860,), (0.3530,))((raw_dataset.data/255).clone().detach().float().to(device))
    else:
